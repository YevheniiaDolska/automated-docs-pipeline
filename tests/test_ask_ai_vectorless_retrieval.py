...
def _import_runtime_module(runtime_root: Path, module_name: str):
    # The source pack (runtime/ask-ai-pack) is canonical; an installed copy
    # (ask-ai-runtime) is optional and may be absent or stale on a fresh clone.
    # Skip runtime roots that carry no source so a missing install never fails
    # the suite -- while the source pack is always exercised.
    module_file = runtime_root.joinpath(*module_name.split(".")).with_suffix(".py")
    if not module_file.is_file():
        pytest.skip(f"runtime source not present: {module_file}")
