...
def rewrite_query_with_history(
    ...
    """Enrich a follow-up question with entities from recent turns for retrieval.

    A short or back-referential question (for example "and in self-hosted?")
    carries little to retrieve on. This prepends salient terms from the most
    recent user turn so the retrieval query stands on its own. Deterministic and
    offline; answer generation still receives the full history separately.
    """
    if not history:
        return question
    q_terms = _question_terms(question)
    words = {tok.lower() for tok in _WORD_RE.findall(question or "")}
    followup_markers = {"it", "its", "this", "that", "these", "those", "they", "them", "one", "ones", "there"}
    starts_with_conjunction = bool(re.match(r"^(and|or|but|also|then)\b", question.strip().lower()))
    is_followup = starts_with_conjunction or bool(words & followup_markers) or len(q_terms) <= 2
    if not is_followup:
        return question
    ...
    for message in reversed(history[-max_turns:]):
        ...
        if role == "user":
            context_terms = _question_terms(str(content))
            if context_terms:
                break
    seen = set(q_terms)
    extra = [term for term in context_terms if term not in seen][:6]
    if not extra:
        return question
...
def build_context(
    ...
    # History-aware rewriting: enrich a follow-up so it retrieves in context.
    # The original question is kept for the response and answer generation.
    retrieval_query = rewrite_query_with_history(question, history)

    normalized_mode = str(retrieval_mode or "auto").strip().lower()
    queries = [retrieval_query]
    if query_decomp_enabled:
        for part in decompose_query(retrieval_query):
    ...
    if rerank_enabled and top and len(top) > max_context_modules and _HAS_CROSS_ENCODER:
        try:
            top = _rerank(retrieval_query, top, top_n=max_context_modules, model_name=rerank_model)
        except (Exception,):  # noqa: BLE001
    ...
    if not top:
        selected_mode = "token"
        top = rank_modules(retrieval_query, modules, max_context_modules)
