...
| Zero-touch screenshot relevance | Metadata-driven screenshot loop: capture plan (`docs/screenshots.capture.yml`) -> automated browser capture (Playwright) -> capture manifest -> auto-built placement manifest -> deterministic insertion by doc + anchor/heading with idempotent update-by-id behavior and `needs_review` queue for ambiguous targets. | Screenshot maintenance consumes expert time -> deterministic visuals with minimal manual intervention. |
...
| QA asset automation | Generates protocol-aware test assets with smart merge preservation, optional company-style adaptation from RAG test index, and optional TestRail/Zephyr export. | QA docs/tests are rewritten each release -> lower churn and faster regression readiness. |
...
| Code-first proofability layer | AST/code-aware traceability index (`imports/calls/config deps`) is built as a pipeline artifact (`reports/code_traceability_index.json`) and strengthens traceability from docs claims to source facts. | Claims are hard to verify in code-first stacks -> higher trust and defensibility. |
