...
def _create_profile_via_wizard(default_scheduler: str, *, require_repo: bool = True) -> tuple[Path, str, str]:
    ...
    product_name = _prompt_with_default("Product name", str(profile["client"].get("product_name", "Product")))
    suggested_id = _slugify_client_id(f"{company_name}-{product_name}") or "client"
    # Auto-generate client_id from company + product to avoid manual intake field.
    client_id = suggested_id
    if not client_id:
    ...
    profile["client"]["product_name"] = product_name
...
def execute_provision(args: argparse.Namespace) -> int:
    ...
    binding_path = write_repo_binding_file(client_repo, args.docsops_dir, profile_path)
    integrity_path = write_integrity_manifest_file(client_repo, args.docsops_dir)
    ...
    print(f"[ok] repo binding file: {binding_path}")
    print(f"[ok] integrity manifest: {integrity_path}")
    if workflow_path:
