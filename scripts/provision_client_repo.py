...
def _ensure_ssg_scaffold(client_repo: Path, docsops_dir: str) -> Path | None:
    ...
    if not isinstance(api_first, dict):
        api_first = {}
    provider = str(api_first.get("docs_provider", "mkdocs")).strip().lower() or "mkdocs"
    docsops_root = client_repo / docsops_dir
    docs_dir = docsops_root / "docs"
    ...
    if not index_path.exists():
        index_path.write_text(
            ...
            "description: \"Entry page for provisioned client documentation and pipeline-generated assets.\"\n"
    ...
    if provider == "mkdocs":
        mkdocs_path = docsops_root / "mkdocs.yml"
        if not mkdocs_path.exists():
            mkdocs_path.write_text(
                "site_name: Client Docs\n"
                "docs_dir: docs\n"
                "nav:\n"
                "  - Home: index.md\n",
                encoding="utf-8",
    ...
    if provider == "docusaurus":
        cfg_path = docsops_root / "docusaurus.config.js"
        if not cfg_path.exists():
            cfg_path.write_text(
                ...
                "  presets: [['classic', { docs: { path: 'docs', routeBasePath: '/' } }]],\n"
                "};\n",
                encoding="utf-8",
            )
        return cfg_path
    ...
    if provider == "hugo":
        cfg_path = docsops_root / "hugo.toml"
        if not cfg_path.exists():
            cfg_path.write_text(
    ...
    if provider == "vitepress":
        cfg_path = docsops_root / "docs" / ".vitepress" / "config.mts"
        ...
        if not cfg_path.exists():
            cfg_path.write_text(
                ...
                "  title: 'Client Docs',\n"
                "  description: 'Provisioned VitePress docs site',\n"
                "})\n",
                encoding="utf-8",
            )
        return cfg_path
    ...
    if provider == "sphinx":
        cfg_path = docsops_root / "docs" / "conf.py"
        cfg_path.parent.mkdir(parents=True, exist_ok=True)
        if not cfg_path.exists():
            cfg_path.write_text(
                "project = 'Client Docs'\n"
        ...
        index_rst = docsops_root / "docs" / "index.rst"
    ...
    if provider == "jekyll":
        cfg_path = docsops_root / "_config.yml"
        if not cfg_path.exists():
            cfg_path.write_text(
                "title: Client Docs\n"
                "description: Provisioned Jekyll docs site\n"
                "theme: minima\n",
                encoding="utf-8",
            )
        return cfg_path
    ...
    if provider == "custom":
        cfg_path = docsops_root / "CUSTOM_SSG_SETUP.md"
        if not cfg_path.exists():
            cfg_path.write_text(
                "# Custom SSG setup\\n\\n"
                "Provisioning detected `docs_provider: custom`.\\n"
                "Add your generator config and point docs root to `docs/`.\\n",
                encoding="utf-8",
            )
        return cfg_path
...
def execute_provision(args: argparse.Namespace) -> int:
    ...
    ssg_path = _ensure_ssg_scaffold(client_repo, args.docsops_dir)
    ...
    if ssg_path is not None:
        print(f"[ok] ssg scaffold ready: {ssg_path}")
    if workflow_path:
