...
Provides a unified interface for MkDocs, Docusaurus, Sphinx, Hugo, and Jekyll site generators.
...
class SiteGenerator(ABC):
    ...
    def detect(project_dir: Path | None = None) -> SiteGenerator:
        ...
        - conf.py (root or docs/) present -> Sphinx
        - _config.yml + Jekyll structure (_layouts or _posts) -> Jekyll
        - hugo.toml (or config.toml/yaml + content/) -> Hugo
        ...
        for cfg in sphinx_configs:
            if (project_dir / cfg).exists():
                return SphinxGenerator()

        jekyll_config = project_dir / "_config.yml"
        if jekyll_config.exists() and (
            (project_dir / "_layouts").exists() or (project_dir / "_posts").exists()
        ):
            return JekyllGenerator()

        if (project_dir / "hugo.toml").exists():
        ...
        for cfg in hugo_alt_configs:
            if (project_dir / cfg).exists() and (project_dir / "content").exists():
                return HugoGenerator()
    ...
    def from_name(name: str) -> SiteGenerator:
        ...
        Args:
            name: Either 'mkdocs', 'docusaurus', 'sphinx', 'hugo', or 'jekyll' (case-insensitive).
        ...
        if normalized == "docusaurus":
            return DocusaurusGenerator()
        if normalized == "sphinx":
            return SphinxGenerator()
        if normalized == "hugo":
            return HugoGenerator()
        if normalized == "jekyll":
            return JekyllGenerator()
        raise ValueError(
            f"Unknown generator '{name}'. Choose 'mkdocs', 'docusaurus', 'sphinx', 'hugo', or 'jekyll'."
        )
...
class MkDocsGenerator(SiteGenerator):
    ...
    def generate_config(self, variables: dict, nav: list) -> str:
        ...
        config = {
            ...
            "site_description": f"{product} documentation powered by VeriOps",
            "site_url": site_url,
            "theme": {
                "name": "material",
                "language": "en",
...
class SphinxGenerator(SiteGenerator):
    ...
    def get_build_command(self) -> list[str]:
        return ["sphinx-build", "-b", "html", "docs", "_build/html"]

    def get_serve_command(self) -> list[str]:
        return ["python3", "-m", "http.server", "8000", "--directory", "_build/html"]
    ...
    def generate_config(self, variables: dict, nav: list) -> str:
        """Generate minimal conf.py content."""
        product = variables.get("product_name", "Documentation")
        return (
            "# Auto-generated by VeriOps\n"
            f"project = '{product} Documentation'\n"
            "extensions = []\n"
    ...
    def build_url_from_path(self, filepath: Path, docs_dir: Path) -> str:
        """Build URL path following Sphinx HTML conventions."""
        rel = filepath.relative_to(docs_dir)
        rel_str = str(rel).replace("\\", "/")
        if rel.name in {"index.rst", "index.md"}:
            parent = str(rel.parent).replace("\\", "/")
            return "" if parent == "." else f"{parent}/"
        return re.sub(r"\.(rst|md)$", ".html", rel_str, flags=re.IGNORECASE)

    def get_nav_update_instructions(self, filepath: str, title: str) -> str:
        return (
            "Add the document to an appropriate toctree in docs/index.rst "
            f"or a section index file:\n  {filepath}"
        )


class HugoGenerator(SiteGenerator):
    """Hugo site generator."""
    ...
    def generate_config(self, variables: dict, nav: list) -> str:
        product = variables.get("product_name", "Documentation")
        base_url = variables.get("docs_url", "https://example.com/")
        return (
            f'baseURL = "{base_url}"\n'
    ...
    def build_url_from_path(self, filepath: Path, docs_dir: Path) -> str:
        rel = filepath.relative_to(docs_dir)
        rel_str = str(rel).replace("\\", "/")
        if rel.name in {"index.md", "_index.md"}:
            parent = str(rel.parent).replace("\\", "/")
            return "" if parent == "." else f"{parent}/"
        url = re.sub(r"\.(md|markdown)$", "", rel_str, flags=re.IGNORECASE)
        return f"{url}/"

    def get_nav_update_instructions(self, filepath: str, title: str) -> str:
        return (
            "Add the page to Hugo menus/section navigation if required:\n"
            f"  {filepath}"
        )


class JekyllGenerator(SiteGenerator):
    ...
    def generate_config(self, variables: dict, nav: list) -> str:
        product = variables.get("product_name", "Documentation")
        base_url = variables.get("docs_url", "")
        return (
            f'title: "{product} Documentation"\n'
            f'url: "{base_url}"\n'
            'markdown: kramdown\n'
    ...
    def build_url_from_path(self, filepath: Path, docs_dir: Path) -> str:
        ...
        if rel.name == "index.md":
            parent = str(rel.parent).replace("\\", "/")
            return "" if parent == "." else f"{parent}/"
        return re.sub(r"\.md$", ".html", rel_str, flags=re.IGNORECASE)

    def get_nav_update_instructions(self, filepath: str, title: str) -> str:
        return (
            "Ensure page is linked in Jekyll navigation includes/layouts:\n"
            f"  {filepath}"
        )
