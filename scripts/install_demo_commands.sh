...
CLAUDE_API_FIRST_SRC="$REPO_ROOT/.claude/commands/api-first-demo.md"
...
if [[ ! -f "$CLAUDE_API_FIRST_SRC" ]]; then
  echo "Missing source command: $CLAUDE_API_FIRST_SRC" >&2
  exit 1
fi
...
cp "$CLAUDE_API_FIRST_SRC" "$HOME/.claude/commands/api-first-demo.md"
...
echo "Installed Claude command: $HOME/.claude/commands/api-first-demo.md"
