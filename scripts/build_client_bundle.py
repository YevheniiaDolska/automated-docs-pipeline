...
def build_managed_instruction_block(docsops_root: str) -> str:
    ...
    lines = [
        ...
        f"3. Run only scripts from `{root}/scripts/` that are required by enabled modules.",
        "4. Block publish if verification fails.",
        "5. Return a short report: changed files, checks passed/failed, publish targets.",
        "",
