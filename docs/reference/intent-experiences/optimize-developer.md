...
# Intent experience: optimize for developer

This page is assembled for the `optimize` intent and the `developer` audience using reusable modules.

```bash
python3 scripts/assemble_intent_experience.py \
  --intent optimize --audience developer --channel docs
```
