...
curl -X POST <https://your-instance.the> product.cloud/webhook/abc123 \
...
1. Create a **Basic Auth credential** with a username and password.
1. Save the workflow.

## Option 3: HMAC signature verification

HMAC (Hash-based Message Authentication Code) verification is the most secure option. The sending service signs each request body with a shared secret, and verifies the signature matches. Services like GitHub, Stripe, and Shopify use HMAC webhooks.

!!! warning "version requirement"
 HMAC verification in the Webhook node requires version 1.40.0 or later. For earlier versions, use a Code node after the Webhook to verify manually.

1. Open the Webhook node settings.
1. Set **Authentication** to **None** (HMAC is verified in a subsequent node).
1. Add a **Code node** after the Webhook node with the following JavaScript:
...
1. Connect the workflow continuation after the Code node.
