...
def _print_compact_output(output: str, *, max_lines: int = 28) -> None:
    """Internal helper for `_print_compact_output`."""
...
def run_first_available(
    ...
    *,
    ...
    """Execute `run_first_available` workflow."""
    ...
    for cmd in candidates:
        ...
        try:
            ...
            return
        except (subprocess.CalledProcessError, RuntimeError, ValueError, TypeError, OSError) as error:  # noqa: BLE001
            last_error = error
            continue
    if last_error:
...
class _SpecBundler:
    ...
    def _resolve_pointer(self, data: Any, pointer: str) -> Any:
        """Internal helper for `_resolve_pointer`."""
    ...
    def _deep_resolve(self, obj: Any, base_dir: Path) -> Any:
        """Internal helper for `_deep_resolve`."""
    ...
    def _collect_schemas_from_file(self, file_path: Path) -> None:
        """Internal helper for `_collect_schemas_from_file`."""
...
def build_sandbox_page_url(repo: Path, docs_provider: str) -> str:
    """Execute `build_sandbox_page_url` workflow."""
...
def sync_playground_sandbox_url(repo: Path, sandbox_base_url: str) -> None:
    """Execute `sync_playground_sandbox_url` workflow."""
    mkdocs_path = repo / "mkdocs.yml"
    if not mkdocs_path.exists():
...
def resolve_mock_base_url(repo: Path, args: argparse.Namespace) -> str:
    """Execute `resolve_mock_base_url` workflow."""
...
def self_verify_stub_coverage(spec_path: Path, stub_file: Path) -> None:
    """Execute `self_verify_stub_coverage` workflow."""
    import yaml
...
def run_one_attempt(
    ...
    """Execute `run_one_attempt` workflow."""
    ...
    if verify_user_path:
        narrator.note(f"Run user-path simulation against {verification_target} endpoint")
        try:
            run(
            ...
            raise RuntimeError(
                f"Step 4/5 failed: user-path verification against {verification_target} endpoint `{mock_base_url}`. "
...
def _resolve_docs_root(runtime_config: Path | None, fallback: str) -> str:
    """Internal helper for `_resolve_docs_root`."""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        choices=["real", "mock"],
        default="mock",
        help="User-path verification mode: real endpoints or mock/sandbox endpoint",
    )
...
if __name__ == "__main__":
    ...
    except (RuntimeError, ValueError, TypeError, OSError) as error:  # pragma: no cover
        print(f"[error] {error}")
        raise SystemExit(1)
