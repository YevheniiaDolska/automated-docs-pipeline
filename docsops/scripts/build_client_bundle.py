...
def _filter_scripts_by_package(include_scripts: list[str], commercial_package: str) -> list[str]:
    """Internal helper for `_filter_scripts_by_package`."""
...
def write_yaml(path: Path, data: dict[str, Any]) -> None:
    """Execute `write_yaml` workflow."""
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", encoding="utf-8") as fh:
        yaml.safe_dump(data, fh, sort_keys=False, allow_unicode=False)


def _ensure_mkdocs_theme_overrides(bundle_root: Path) -> None:
    """Ensure MkDocs theme custom_dir exists when configured.

    Some client repos keep `theme.custom_dir: overrides` in mkdocs.yml.
    If the directory is missing, `mkdocs build` fails. The bundle must always
    be build-ready without manual mkdir steps.
    """
    mkdocs_path = bundle_root / "mkdocs.yml"
    if not mkdocs_path.exists():
        return
    try:
        cfg = read_yaml(mkdocs_path)
    except (OSError, ValueError, TypeError):
        return
    theme = cfg.get("theme", {})
    if not isinstance(theme, Mapping):
        return
    custom_dir_raw = str(theme.get("custom_dir", "")).strip()
    if not custom_dir_raw:
        return
    custom_dir = Path(custom_dir_raw)
    if custom_dir.is_absolute():
        target = custom_dir
    else:
        target = (bundle_root / custom_dir).resolve()
...
def copy_into_bundle(rel_path: str, bundle_root: Path) -> None:
    """Execute `copy_into_bundle` workflow."""
...
def copy_path_into_bundle(rel_path: str, bundle_root: Path) -> None:
    """Execute `copy_path_into_bundle` workflow."""
    src = REPO_ROOT / rel_path
    if not src.exists():
...
def _append_unique(items: list[str], value: str) -> None:
    """Internal helper for `_append_unique`."""
    if value not in items:
        items.append(value)


def _template_paths_for_package(commercial_package: str) -> list[str]:
    """Internal helper for `_template_paths_for_package`."""
...
def _normalize_i18n_locales(raw_locales: Any) -> list[str]:
    """Internal helper for `_normalize_i18n_locales`."""
    values: list[str] = []
    if isinstance(raw_locales, list):
        values = [str(v).strip().lower().replace("_", "-") for v in raw_locales]
    normalized: list[str] = []
    seen: set[str] = set()
    for value in values:
        if not value:
        ...
        if not all(part.isalnum() for part in parts):
            continue
        if len(parts[0]) < 2 or len(parts[0]) > 3:
            continue
        if value in seen:
            continue
        seen.add(value)
...
def _build_i18n_bundle_config(runtime_cfg: dict[str, Any]) -> dict[str, Any]:
    """Internal helper for `_build_i18n_bundle_config`."""
    base: dict[str, Any] = {}
    source_i18n_path = REPO_ROOT / "i18n.yml"
    if source_i18n_path.exists():
        try:
            loaded = read_yaml(source_i18n_path)
            if isinstance(loaded, dict):
                base = loaded
        except (ValueError, OSError, TypeError):
            base = {}

    runtime_i18n = runtime_cfg.get("i18n", {})
    if not isinstance(runtime_i18n, dict):
        runtime_i18n = {}

    default_language = str(runtime_i18n.get("default_language", "en")).strip().lower() or "en"
    if default_language != "en":
        default_language = "en"

    locales = _normalize_i18n_locales(runtime_i18n.get("locales", []))
    if default_language not in locales:
        locales.insert(0, default_language)
    locales = [default_language] + [loc for loc in locales if loc != default_language]

    base_languages = {}
    raw_languages = base.get("languages", [])
    if isinstance(raw_languages, list):
        for entry in raw_languages:
            ...
            if locale:
                base_languages[locale] = entry

    languages: list[dict[str, Any]] = []
    for locale in locales:
        existing = base_languages.get(locale, {})
        language_entry = {
            "locale": locale,
            "name": str(existing.get("name", _LOCALE_NAME_FALLBACKS.get(locale, locale.upper()))),
            "build": bool(existing.get("build", True)),
        }
        seo_overrides = existing.get("seo_overrides")
        if isinstance(seo_overrides, dict) and seo_overrides:
            language_entry["seo_overrides"] = seo_overrides
    ...
    return {
        ...
        "docs_structure": str(base.get("docs_structure", "folder")),
        "languages": languages,
...
def _apply_rag_package_policy_to_runtime(runtime_cfg: dict[str, Any], commercial_package: str) -> None:
    """Internal helper for `_apply_rag_package_policy_to_runtime`."""
    ...
    if isinstance(rag_runtime, dict):
        rag_runtime["enabled"] = rag_enabled

    rag_slo = runtime_cfg.get("rag_slo")
    if isinstance(rag_slo, dict):
        rag_slo["enabled"] = rag_enabled


def _is_strict_local_runtime(runtime_cfg: dict[str, Any]) -> bool:
    """Internal helper for `_is_strict_local_runtime`."""
...
def _apply_strict_local_defaults(runtime_cfg: dict[str, Any]) -> None:
    """Internal helper for `_apply_strict_local_defaults`."""
    ...
    if isinstance(api_first, Mapping):
        ...
        api_first_cfg["verification_target"] = "mock"
        api_first_cfg["verification_base_url"] = str(
            api_first_cfg.get("mock_base_url", "http://localhost:4010/v1")
        )
...
def _sha256_file(path: Path) -> str:
    """Internal helper for `_sha256_file`."""
    h = hashlib.sha256()
...
def _load_tier_a_modules() -> list[str]:
    """Internal helper for `_load_tier_a_modules`."""
...
def _find_compiled_artifact(artifacts_dir: Path, module_name: str) -> Path | None:
    """Internal helper for `_find_compiled_artifact`."""
...
def _detect_compile_platform() -> str:
    """Internal helper for `_detect_compile_platform`."""
...
def _attempt_auto_compile_protected_modules(artifacts_dir: Path) -> bool:
    ...
    if completed.returncode != 0:
        ...
        if "cython is not installed" in out_text:
            install_cmd = [sys.executable, "-m", "pip", "install", "cython"]
...
def _apply_hardening_bundle_protection(
    *,
    ...
    """Internal helper for `_apply_hardening_bundle_protection`."""
    security = runtime_cfg.get("security", {})
    if not isinstance(security, Mapping):
        return
    profile_runtime = profile.get("runtime", {})
    explicit_security_cfg = isinstance(profile_runtime, Mapping) and isinstance(profile_runtime.get("security"), Mapping)

    hardening_profile = str(security.get("hardening_profile", "production")).strip().lower()
    # Backward-compatible behavior for minimal profiles/tests:
    # if runtime.security is not explicitly defined in profile, do not enforce compiled-only hardening.
    default_hardening = hardening_profile == "production" and explicit_security_cfg
    require_protected = bool(security.get("require_protected_modules", default_hardening))
    auto_compile = bool(security.get("auto_compile_protected_modules", default_hardening))
    anti_tamper = bool(security.get("anti_tamper_enforced", default_hardening))
    protect_mode_default = "compiled" if explicit_security_cfg else "source"
    protect_mode = str(security.get("protect_mode", protect_mode_default)).strip().lower()
    if not explicit_security_cfg:
    ...
    if protect_mode == "compiled":
        ...
        if missing and auto_compile:
            _attempt_auto_compile_protected_modules(artifacts_dir)
            missing = []
            for module_name in tier_a:
                artifact = _find_compiled_artifact(artifacts_dir, module_name)
                if artifact is None:
...
def render_template(path: Path, replacements: dict[str, str]) -> str:
    """Execute `render_template` workflow."""
...
def build_runtime_config(profile: dict[str, Any]) -> dict[str, Any]:
    """Execute `build_runtime_config` workflow."""
    ...
    return {
        ...
        "api_first": runtime.get(
            "api_first",
            {
                ...
                "verification_target": "real",
                "verification_base_url": "https://api.example.com/v1",
        ...
        "finalize_gate": runtime.get(
            "finalize_gate",
            {
                ...
                "reports_dir": "reports",
                "lint_command": "python3 docsops/scripts/lint_code_snippets.py {docs_root}",
                "max_iterations": 5,
                "continue_on_error": True,
                "auto_fix_commands": [
                    "python3 docsops/scripts/normalize_docs.py {docs_root}",
                    "python3 docsops/scripts/seo_geo_optimizer.py {docs_root} --fix",
                ],
                "llm_fix_command": "",
...
def build_licensing_infrastructure(profile: dict[str, Any], runtime_cfg: dict[str, Any], bundle_root: Path) -> None:
    ...
    def _resolve_repo_path(raw: str) -> Path:
        """Internal helper for `_resolve_repo_path`."""
        candidate = Path(raw)
    ...
    has_explicit_licensing_cfg = isinstance(licensing, Mapping) and bool(licensing)
    ...
    # - explicit licensing config -> require signed JWT by default
    # - no licensing section (tests/minimal local builds) -> allow placeholder
    require_signed_jwt = bool(licensing.get("require_signed_jwt", has_explicit_licensing_cfg))
    ...
    require_pack_for_premium = bool(
        security_cfg.get("require_pack_for_premium", str(security_cfg.get("hardening_profile", "production")).strip().lower() == "production")
    )
    premium_plan = plan in {"professional", "enterprise"}
    if not license_key and auto_pack and client_id and require_pack_for_premium and premium_plan:
        license_key = _default_bundle_license_key(profile)
        print(f"[license] using deterministic bundle license key for capability pack: env={license_key_env}")
    ...
    generate_pack_script = REPO_ROOT / "build" / "generate_pack.py"
    if require_pack_for_premium and premium_plan and not pack_path.exists():
        if generate_pack_script.exists():
            raise RuntimeError(
                "Capability pack is required for premium features but was not generated. "
                f"Provide {license_key_env} or licensing.license_key, or disable require_pack_for_premium."
            )
        print(
            "[license] premium pack enforcement skipped: build/generate_pack.py is missing in current source tree"
        )


def build_licensed_files(profile: dict[str, Any], bundle_root: Path) -> None:
    """Execute `build_licensed_files` workflow."""
...
def _str_or_default(value: Any, default: str) -> str:
    """Internal helper for `_str_or_default`."""
    text = str(value).strip() if value is not None else ""
    return text or default


def _contract_replacements(profile: dict[str, Any], agreement_type: str) -> dict[str, str]:
    """Internal helper for `_contract_replacements`."""
    client_cfg = profile.get("client", {})
    legal_cfg = profile.get("legal", {})
    licensing_cfg = profile.get("licensing", {})
    contracts_cfg = legal_cfg.get("contracts", {}) if isinstance(legal_cfg, Mapping) else {}
    if not isinstance(contracts_cfg, Mapping):
        contracts_cfg = {}

    today = dt.date.today()
    effective_date = _str_or_default(contracts_cfg.get("effective_date"), today.isoformat())

    provider_name = _str_or_default(contracts_cfg.get("provider_legal_name"), "VeriDoc Inc.")
    provider_entity = _str_or_default(contracts_cfg.get("provider_entity_type"), "Corporation")
    provider_jurisdiction = _str_or_default(contracts_cfg.get("provider_jurisdiction"), "Delaware, USA")
    provider_address = _str_or_default(contracts_cfg.get("provider_address"), "[Provider Address]")

    client_name = _str_or_default(client_cfg.get("company_name"), "Client Company")
    client_entity = _str_or_default(contracts_cfg.get("client_entity_type"), "Corporation")
    client_jurisdiction = _str_or_default(contracts_cfg.get("client_jurisdiction"), "[Client Jurisdiction]")
    client_address = _str_or_default(contracts_cfg.get("client_address"), "[Client Address]")

    credit_validity_days = int(contracts_cfg.get("credit_validity_days", 90) or 90)
    retainer_enabled = bool(contracts_cfg.get("retainer_enabled", False))
    retainer_monthly = int(contracts_cfg.get("retainer_monthly_usd", 0) or 0)
    retainer_hours = int(contracts_cfg.get("retainer_included_hours", 0) or 0)
    retainer_overage = int(contracts_cfg.get("retainer_overage_usd_per_hour", 0) or 0)
    retainer_scope = _str_or_default(
        contracts_cfg.get("retainer_scope"),
        "Post-implementation support, remediation, incremental documentation updates, and agreed automation adjustments.",
    )
    retainer_response = _str_or_default(contracts_cfg.get("retainer_response_sla"), "Within two (2) business days.")
    retainer_rollover = _str_or_default(contracts_cfg.get("retainer_rollover_policy"), "Unused hours expire at month end.")
    retainer_suspend_days = int(contracts_cfg.get("retainer_overdue_suspend_days", 7) or 7)
    retainer_notice_days = int(contracts_cfg.get("retainer_termination_notice_days", 30) or 30)
    retainer_degrade_grace_days = int(contracts_cfg.get("retainer_degrade_grace_days", 7) or 7)
    governing_law = _str_or_default(contracts_cfg.get("governing_law"), "Delaware, USA")
    venue = _str_or_default(contracts_cfg.get("venue"), "Wilmington, Delaware, USA")
    payment_terms = _str_or_default(
        contracts_cfg.get("payment_terms"),
        "100% due upfront within five (5) business days of signature.",
    )
    pilot_start = _str_or_default(contracts_cfg.get("pilot_start_date"), effective_date)
    pilot_end = _str_or_default(
        contracts_cfg.get("pilot_end_date"),
        (today + dt.timedelta(days=21)).isoformat(),
    )

    repo_identifier = _str_or_default(
        contracts_cfg.get("repository_identifier"),
        _str_or_default(client_cfg.get("id"), "client-repo"),
    )

    full_price = int(contracts_cfg.get("full_price_usd", 15000) or 15000)
    rag_addon = int(contracts_cfg.get("rag_addon_price_usd", 10000) or 10000)
    pilot_price = int(contracts_cfg.get("pilot_price_usd", 5000) or 5000)
    full_rag_price = full_price + rag_addon
    full_after_pilot = max(0, full_price - pilot_price)
    full_rag_after_pilot = max(0, full_rag_price - pilot_price)

    if agreement_type == "pilot":
        agreement_name = "Pilot Services Agreement"
        project_scope = (
            "21-day single-repository pilot with full docs-ops functionality, "
            "including knowledge preparation for RAG, but excluding production retrieval-time RAG runtime."
        )
        total_fee = pilot_price
    elif agreement_type == "full":
        agreement_name = "Full Implementation Services Agreement"
        project_scope = (
            "Full Auto-Doc Pipeline implementation without production retrieval-time RAG runtime."
        )
        total_fee = full_price
    else:
        agreement_name = "Full + RAG Implementation Services Agreement"
        project_scope = (
            "Full Auto-Doc Pipeline implementation with production retrieval-time RAG runtime and advanced retrieval layers."
        )
        total_fee = full_rag_price

    return {
        "AGREEMENT_NAME": agreement_name,
        "EFFECTIVE_DATE": effective_date,
        "PROVIDER_LEGAL_NAME": provider_name,
        "PROVIDER_ENTITY_TYPE": provider_entity,
        "PROVIDER_JURISDICTION": provider_jurisdiction,
        "PROVIDER_ADDRESS": provider_address,
        "CLIENT_LEGAL_NAME": client_name,
        "CLIENT_ENTITY_TYPE": client_entity,
        ...
        "PILOT_PRICE_USD": f"{pilot_price:,}",
        "FULL_PRICE_USD": f"{full_price:,}",
        "RAG_ADDON_PRICE_USD": f"{rag_addon:,}",
        "FULL_RAG_PRICE_USD": f"{full_rag_price:,}",
        "FULL_AFTER_PILOT_USD": f"{full_after_pilot:,}",
        "FULL_RAG_AFTER_PILOT_USD": f"{full_rag_after_pilot:,}",
        "TOTAL_FEE_USD": f"{total_fee:,}",
        "CREDIT_VALIDITY_DAYS": str(credit_validity_days),
        "RETAINER_ENABLED": "enabled" if retainer_enabled else "disabled",
        "RETAINER_MONTHLY_USD": f"{retainer_monthly:,}",
        "RETAINER_INCLUDED_HOURS": str(retainer_hours),
        "RETAINER_OVERAGE_USD_PER_HOUR": f"{retainer_overage:,}",
        "RETAINER_SCOPE": retainer_scope,
        "RETAINER_RESPONSE_SLA": retainer_response,
        "RETAINER_ROLLOVER_POLICY": retainer_rollover,
        "RETAINER_OVERDUE_SUSPEND_DAYS": str(retainer_suspend_days),
        "RETAINER_TERMINATION_NOTICE_DAYS": str(retainer_notice_days),
        "RETAINER_DEGRADE_GRACE_DAYS": str(retainer_degrade_grace_days),
        "RESTRICTED_USE_MONTHS": str(int(contracts_cfg.get("restricted_use_months", 36) or 36)),
        "GOVERNING_LAW_STATE_OR_COUNTRY": governing_law,
        "VENUE_CITY_AND_STATE_OR_COUNTRY": venue,
        "PROVIDER_SIGNATORY_NAME": _str_or_default(contracts_cfg.get("provider_signatory_name"), ""),
        "PROVIDER_SIGNATORY_TITLE": _str_or_default(contracts_cfg.get("provider_signatory_title"), ""),
        "PROVIDER_SIGNATORY_EMAIL": _str_or_default(contracts_cfg.get("provider_signatory_email"), ""),
        "CLIENT_SIGNATORY_NAME": _str_or_default(contracts_cfg.get("client_signatory_name"), ""),
        "CLIENT_SIGNATORY_TITLE": _str_or_default(contracts_cfg.get("client_signatory_title"), ""),
        "CLIENT_SIGNATORY_EMAIL": _str_or_default(
            contracts_cfg.get("client_signatory_email"),
            _str_or_default(client_cfg.get("contact_email"), ""),
        ),
        "LICENSING_PLAN": _str_or_default(licensing_cfg.get("plan"), "professional"),
    }


def build_sales_contracts(profile: dict[str, Any], bundle_root: Path) -> None:
    """Execute `build_sales_contracts` workflow."""
    legal_dir = bundle_root / "legal"
    signnow_dir = legal_dir / "signnow"
    pdf_dir = signnow_dir / "pdf"
    legal_dir.mkdir(parents=True, exist_ok=True)
    signnow_dir.mkdir(parents=True, exist_ok=True)
    pdf_dir.mkdir(parents=True, exist_ok=True)
    master_template = REPO_ROOT / "templates" / "legal" / "MASTER-SERVICES-AGREEMENT.template.md"
    if not master_template.exists():
        print("[warn] legal master template not found; contract generation skipped")
        return

    contracts = [
        ("pilot", "PILOT-SERVICES-AGREEMENT.md"),
        ("full", "FULL-IMPLEMENTATION-SERVICES-AGREEMENT.md"),
        ("full+rag", "FULL-PLUS-RAG-IMPLEMENTATION-SERVICES-AGREEMENT.md"),
    ]
    for agreement_type, output_name in contracts:
        replacements = _contract_replacements(profile, agreement_type)
        rendered = render_template(master_template, replacements)
        generated_path = legal_dir / output_name
        generated_path.write_text(rendered, encoding="utf-8")
        # SignNow upload variant: plain contract body without YAML frontmatter.
        signnow_text = rendered
        if signnow_text.startswith("---\n"):
            _, _, rest = signnow_text.partition("\n---\n")
            if rest:
                signnow_text = rest.lstrip("\n")
        signnow_path = signnow_dir / output_name
        signnow_path.write_text(signnow_text, encoding="utf-8")
        _render_contract_pdf(signnow_text, pdf_dir / output_name.replace(".md", ".pdf"))


def _render_contract_pdf(markdown_text: str, output_pdf: Path) -> None:
    """Internal helper for `_render_contract_pdf`."""
...
def build_vale_config(profile: dict[str, Any], bundle_root: Path) -> None:
    """Execute `build_vale_config` workflow."""
...
def build_managed_instruction_block(docsops_root: str) -> str:
    """Execute `build_managed_instruction_block` workflow."""
...
def upsert_managed_block(file_path: Path, block_text: str) -> None:
    """Execute `upsert_managed_block` workflow."""
...
def build_llm_instruction_files(profile: dict[str, Any], bundle_root: Path) -> None:
    """Execute `build_llm_instruction_files` workflow."""
    bundle_cfg = profile.get("bundle", {})
    ...
    ollama_dst = bundle_root / "OLLAMA_SYSTEM_PROMPT.md"
    mandatory_dst = bundle_root / "MANDATORY_RUN_GATES.md"
    shutil.copy2(codex_src, codex_dst)
    shutil.copy2(claude_src, claude_dst)
    shutil.copy2(local_src, local_dst)
    shutil.copy2(local_src, ollama_dst)
    mandatory_src = REPO_ROOT / "MANDATORY_RUN_GATES.md"
    if mandatory_src.exists():
        shutil.copy2(mandatory_src, mandatory_dst)
    ...
    if bool(llm_cfg.get("inject_managed_block", True)):
        ...
        upsert_managed_block(ollama_dst, block)


def _cron_day_to_number(day: str) -> str:
    """Internal helper for `_cron_day_to_number`."""
    mapping = {
        "sunday": "0",
        "monday": "1",
        "tuesday": "2",
...
def build_automation_files(profile: dict[str, Any], bundle_root: Path) -> None:
    """Execute `build_automation_files` workflow."""
    bundle_cfg = profile.get("bundle", {})
    llm_cfg = bundle_cfg.get("llm", {})
...
def _append_env(lines: list[str], key: str, value: str, comment: str) -> None:
    """Internal helper for `_append_env`."""
    lines.append(f"# {comment}")
    lines.append(f"{key}={value}")
    lines.append("")


def _default_bundle_license_key(profile: Mapping[str, Any]) -> str:
    """Internal helper for `_default_bundle_license_key`."""
    client = profile.get("client", {})
    if not isinstance(client, Mapping):
        client = {}
    client_id = str(client.get("id", "")).strip() or "client"
    tenant_id = str(client.get("tenant_id", client_id)).strip() or client_id
    company_domain = str(client.get("company_domain", "")).strip().lower() or "local"
    seed = f"{client_id}|{tenant_id}|{company_domain}|veriops-pack-v1"
    digest = hashlib.sha256(seed.encode("utf-8")).hexdigest()
    return f"vk_{digest[:40]}"


def build_local_env_template(
    profile: dict[str, Any] | dict[str, object],
    runtime_cfg: dict[str, Any] | Path,
    bundle_root: Path | None = None,
) -> None:
    # Backward-compatible call form:
    # build_local_env_template(runtime_cfg, bundle_root)
    """Execute `build_local_env_template` workflow."""
    if bundle_root is None and isinstance(runtime_cfg, Path):
        bundle_root = runtime_cfg
        runtime_cfg = profile
        profile = {}
    if bundle_root is None:
        raise TypeError("bundle_root is required")
    ...
    if not isinstance(licensing, Mapping):
        licensing = {}
    has_explicit_licensing_cfg = bool(licensing)
    security_cfg = runtime_cfg.get("security", {})
    if not isinstance(security_cfg, Mapping):
        security_cfg = {}
    require_pack_for_premium = bool(
    ...
    premium_plan = plan in {"professional", "enterprise"}
    expose_license_key_env = bool(
        licensing.get("expose_license_key_env", (not has_explicit_licensing_cfg) or (require_pack_for_premium and premium_plan))
    )
    expose_dev_plan_override_env = bool(
        licensing.get("expose_dev_plan_override_env", not has_explicit_licensing_cfg)
    )
    license_key_env = str(licensing.get("license_key_env", "VERIOPS_LICENSE_KEY")).strip() or "VERIOPS_LICENSE_KEY"
    ...
    default_license_key = str(licensing.get("license_key", "")).strip()
    if not default_license_key and require_pack_for_premium and premium_plan:
        default_license_key = _default_bundle_license_key(profile)
    ...
    if expose_license_key_env:
        _append_env(
            ...
            license_key_env,
            default_license_key,
            "License key used for encrypted capability pack decryption (required for premium features).",
        )
    ...
    if isinstance(branding_cfg, Mapping) and bool(branding_cfg.get("enabled", False)):
        _append_env(
            lines,
            str(branding_cfg.get("referral_code_env", "VERIDOC_REFERRAL_CODE")).strip() or "VERIDOC_REFERRAL_CODE",
            "",
            "Optional referral code for Powered by VeriDoc badge links.",
        )
...
def create_bundle(profile_path: Path) -> Path:
    """Execute `create_bundle` workflow."""
    ...
    modules_cfg = runtime_cfg.get("modules", {})
    if not isinstance(modules_cfg, Mapping):
        modules_cfg = {}

    def _module_enabled(name: str, default: bool = True) -> bool:
        """Internal helper for `_module_enabled`."""
        return bool(modules_cfg.get(name, default))
    ...
    required_scripts.append("scripts/run_prompt_pipeline.py")
    required_scripts.append("scripts/new_doc.py")
    required_scripts.append("scripts/audit_bundle_docs_quality.py")
    required_scripts.append("scripts/pipeline_event_bus.py")
    required_scripts.append("scripts/validate_event_contract.py")
    required_scripts.append("scripts/llm_egress.py")
    required_scripts.append("scripts/flow_feedback.py")
    required_scripts.append("scripts/run_weekly_gap_batch.py")
    required_scripts.append("scripts/consolidate_reports.py")
    required_scripts.append("scripts/generate_audit_scorecard.py")
    required_scripts.append("scripts/compile_doc_overview.py")
    required_scripts.append("scripts/generate_release_docs_pack.py")
    required_scripts.append("scripts/build_all_intent_experiences.py")
    required_scripts.append("scripts/assemble_intent_experience.py")
    # Keep multi-protocol flow available in all bundles for zero-config prompt routing.
    ...
    if _module_enabled("kpi_sla", True):
        required_scripts.append("scripts/generate_kpi_wall.py")
        required_scripts.append("scripts/evaluate_kpi_sla.py")
    if _module_enabled("knowledge_validation", True) or _module_enabled("rag_optimization", True):
        required_scripts.append("scripts/enforce_rag_optimization_layer.py")
        required_scripts.append("scripts/rag_reindex_lifecycle.py")
        required_scripts.append("scripts/generate_embeddings.py")
    ...
    if _module_enabled("code_intelligence", False):
        required_scripts.append("scripts/extract_code_knowledge_graph.py")
    if _module_enabled("ontology_graph", True):
        required_scripts.append("scripts/generate_knowledge_graph_jsonld.py")
    if _module_enabled("retrieval_evals", True):
        required_scripts.append("scripts/run_retrieval_evals.py")
    if _module_enabled("terminology_management", True):
        required_scripts.append("scripts/sync_project_glossary.py")
    if isinstance(branding_cfg, Mapping) and bool(branding_cfg.get("enabled", False)):
    ...
    include_paths = [str(rel) for rel in bundle_cfg.get("include_paths", [])]
    for required_path in ("config/event_contract.yml",):
        if required_path not in include_paths:
            include_paths.append(required_path)
    ...
    if (REPO_ROOT / ".spectral.yml").exists():
        _append_unique(include_paths, ".spectral.yml")
    ...
    _ensure_mkdocs_theme_overrides(bundle_root)

    write_yaml(bundle_root / "i18n.yml", _build_i18n_bundle_config(runtime_cfg))
    ...
    operator_note = {
        ...
        "local_use": {
            "llm_instructions": ["AGENTS.md", "CLAUDE.md", "LOCAL_MODEL.md", "MANDATORY_RUN_GATES.md"],
            "runtime_config": "config/client_runtime.yml",
            "policy_pack": "policy_packs/selected.yml",
            "automation_runbook": "ops/runbook.md",
            "sales_contracts": [
                "legal/PILOT-SERVICES-AGREEMENT.md",
                "legal/FULL-IMPLEMENTATION-SERVICES-AGREEMENT.md",
                "legal/FULL-PLUS-RAG-IMPLEMENTATION-SERVICES-AGREEMENT.md",
            ],
            "sales_contracts_signnow": [
                "legal/signnow/PILOT-SERVICES-AGREEMENT.md",
                "legal/signnow/FULL-IMPLEMENTATION-SERVICES-AGREEMENT.md",
                "legal/signnow/FULL-PLUS-RAG-IMPLEMENTATION-SERVICES-AGREEMENT.md",
            ],
            "sales_contracts_signnow_pdf": [
                "legal/signnow/pdf/PILOT-SERVICES-AGREEMENT.pdf",
                "legal/signnow/pdf/FULL-IMPLEMENTATION-SERVICES-AGREEMENT.pdf",
                "legal/signnow/pdf/FULL-PLUS-RAG-IMPLEMENTATION-SERVICES-AGREEMENT.pdf",
            ],
    ...
    build_sales_contracts(profile, bundle_root)
