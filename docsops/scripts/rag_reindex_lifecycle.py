...
def _resolve_script_path(repo_root: Path, script_name: str) -> Path:
    """Internal helper for `_resolve_script_path`."""
    candidates = [
...
def _copy_if_exists(source: Path, target: Path) -> bool:
    """Internal helper for `_copy_if_exists`."""
...
def _promote_pointer(index_path: Path, manifest: dict[str, Any]) -> None:
    """Internal helper for `_promote_pointer`."""
...
def _manifest_for_version(version: str, version_dir: Path, index_path: Path, provider: str) -> dict[str, Any]:
    """Internal helper for `_manifest_for_version`."""
