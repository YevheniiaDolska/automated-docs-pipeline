...
class DocumentCreator:
    ...
    def __init__(
        ...
        if self.locale and self.i18n_config:
            ...
            self.variables = self.load_variables()
        self._variables_before = self._read_variables_snapshot()
    ...
    def _append_generation_trace(self, event: str, payload: dict) -> None:
        """Append generation lifecycle event for process-level audit."""
        trace_path = Path("reports/generation_trace_report.json")
        ...
        trace_path.write_text(json.dumps(current, ensure_ascii=True, indent=2) + "\n", encoding="utf-8")
        status = "info"
        if event in {"doc_created", "template_created"}:
            status = "finished"
        emit_event(
            repo_root=Path.cwd(),
            event=event,
    ...
    def _read_variables_snapshot(self) -> dict:
        """Internal helper for `_read_variables_snapshot`."""
        path = self.base_dir / "_variables.yml"
        if not path.exists():
            return {}
        try:
    ...
    def _flatten_keys(self, obj: dict, prefix: str = "") -> set[str]:
        """Internal helper for `_flatten_keys`."""
        out: set[str] = set()
        for key, value in obj.items():
            name = f"{prefix}.{key}" if prefix else str(key)
            out.add(name)
            if isinstance(value, dict):
                out.update(self._flatten_keys(value, name))
        return out

    def _trace_variables_delta(self) -> None:
        """Internal helper for `_trace_variables_delta`."""
        before = self._flatten_keys(self._variables_before if isinstance(self._variables_before, dict) else {})
        after_snapshot = self._read_variables_snapshot()
        after = self._flatten_keys(after_snapshot if isinstance(after_snapshot, dict) else {})
        added = sorted(after - before)
        if added:
            self._append_generation_trace(
                "variables_added",
                {
                    "variables_file": str(self.base_dir / "_variables.yml"),
                    "added_keys": added,
                },
            )
        self._variables_before = after_snapshot

    def _ensure_stripe_quality_template_body(self, body: str, content_type: str) -> str:
        """Normalize auto-created template body to Stripe-quality minimum structure."""
        text = (body or "").strip("\n")
        if not text:
            text = (
                "[Topic] is [one-sentence definition]. This guide shows how to achieve [concrete outcome] "
                "with clear validation and production-ready checks.\n\n"
                "## Quick example\n\n"
                "```bash\n"
                ...
                "- [Requirement 2 with access scope]\n\n"
                "## Step 1: [Action]\n\n"
                ...
                "## Performance notes\n\n"
                "Limits, throughput, and timeout guidance.\n\n"
                "## Security notes\n\n"
                "Authentication, authorization, and data validation controls.\n\n"
                "## Next steps\n\n"
                "- [Related how-to]\n"
                "- [Related reference]\n"
            )
        if "## Quick example" not in text and "```" in text:
            text = text.replace("\n## ", "\n\n## ", 1)
            text = f"{text}\n\n## Quick example\n\n```bash\ncommand --with-real-flags\n```\n"
        required_sections = [
            "## Error handling",
    ...
    def _ensure_template_file(self, source: TemplateSource, fallback: dict) -> Path:
        """Require expected template file for template-first generation."""
        path = self.templates_dir / source.file_name
        if path.exists():
            self._append_generation_trace(
                "template_used",
                {
                    "template_path": str(path),
                    "template_name": source.file_name,
                },
            )
            return path
        self._append_generation_trace(
        ...
        frontmatter = {
            "title": source.file_name.replace(".md", "").replace("-", " ").title(),
            "description": str(fallback.get("description_template", source.default_description_template)),
            "content_type": str(fallback.get("content_type", "reference")),
            "product": "both",
            "tags": [],
        }
        body = self._ensure_stripe_quality_template_body(
            str(fallback.get("body", "")).lstrip("\n"),
            str(fallback.get("content_type", "reference")),
        )
    ...
    def _hydrate_from_template_file(
        self,
        source: TemplateSource,
        fallback: dict,
    ) -> dict:
        """Load template body from templates/*.md and merge with built-in defaults."""
        path = self._ensure_template_file(source, fallback)
        raw = path.read_text(encoding="utf-8")
        frontmatter, body = self._split_frontmatter(raw)
        merged = dict(fallback)
        if body.strip():
            merged["body"] = body
        merged["content_type"] = str(
            frontmatter.get("content_type", merged.get("content_type", "reference"))
        ).strip() or str(merged.get("content_type", "reference"))
        merged["description_template"] = str(
            frontmatter.get(
                "description",
                merged.get("description_template", source.default_description_template),
            )
        ).strip() or str(merged.get("description_template", source.default_description_template))
        return merged
...
"""
        ...
        file_sources: dict[str, TemplateSource] = {
            "tutorial": TemplateSource("tutorial.md", "Learn how to {topic} with {product_name}"),
            "how-to": TemplateSource("how-to.md", "Step-by-step guide to {topic}"),
            "concept": TemplateSource("concept.md", "Understanding {topic} in {product_name}"),
            "reference": TemplateSource("reference.md", "Technical reference for {topic}"),
            "troubleshooting": TemplateSource("troubleshooting.md", "Troubleshooting {topic} issues"),
            "api": TemplateSource("api-reference.md", "API reference for {topic}"),
        }
        for doc_type, source in file_sources.items():
            if doc_type in templates:
                templates[doc_type] = self._hydrate_from_template_file(source, templates[doc_type])
        return templates
    ...
    def create_document(
        ...
        if not output_path:
            ...
            if not output_path.is_absolute():
                ...
                # Keep repo-relative paths that already include configured docs_dir.
        ...
        self._append_generation_trace(
            "doc_created",
            {
                ...
                "content_type": str(template.get("content_type", "")),
            },
        )
    ...
    def update_navigation(self, file_path: str, title: str, doc_type: str) -> Any:
    ...
    def validate_document(self, file_path: Path) -> Any:
