...
def _package_from_payload(payload: dict[str, Any]) -> str:
    """Internal helper for `_package_from_payload`."""
...
def _scope_section(package: str, findings: list[dict[str, Any]]) -> dict[str, Any]:
    """Internal helper for `_scope_section`."""
...
def _roadmap_90_days(findings: list[dict[str, Any]], top_gaps: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Internal helper for `_roadmap_90_days`."""
...
def _executive_narrative(payload: dict[str, Any]) -> dict[str, str]:
    """Internal helper for `_executive_narrative`."""
...
def _iter_docs(docs_dir: Path) -> list[Path]:
    """Internal helper for `_iter_docs`."""
...
def _extract_frontmatter(content: str) -> dict[str, Any]:
    """Internal helper for `_extract_frontmatter`."""
...
def _parse_iso_date(raw: str) -> datetime | None:
    """Internal helper for `_parse_iso_date`."""
    value = str(raw).strip()
    if not value:
        return None
...
def _freshness_metrics(docs_dir: Path, stale_days: int) -> dict[str, Any]:
    """Internal helper for `_freshness_metrics`."""
...
def _load_openapi(spec_path: Path) -> dict[str, Any]:
    """Internal helper for `_load_openapi`."""
...
def _collect_operations(spec: dict[str, Any]) -> list[dict[str, str]]:
    """Internal helper for `_collect_operations`."""
...
def _api_coverage_metrics(docs_dir: Path, spec_path: Path) -> dict[str, Any]:
    """Internal helper for `_api_coverage_metrics`."""
...
def _examples_reliability_metrics(
    ...
    """Internal helper for `_examples_reliability_metrics`."""
...
def _drift_metrics(reports_dir: Path) -> dict[str, Any]:
    """Internal helper for `_drift_metrics`."""
...
def _infer_feature_key(path: Path, fm: dict[str, Any], content_type: str) -> str:
    """Internal helper for `_infer_feature_key`."""
...
def _layer_completeness_metrics(docs_dir: Path, policy_pack: Path | None) -> dict[str, Any]:
    """Internal helper for `_layer_completeness_metrics`."""
...
def _retrieval_metrics(reports_dir: Path) -> dict[str, Any]:
    """Internal helper for `_retrieval_metrics`."""
...
def _terminology_metrics(docs_dir: Path, glossary_path: Path, reports_dir: Path) -> dict[str, Any]:
    """Internal helper for `_terminology_metrics`."""
    glossary = {}
...
def _load_assumptions(path: Path | None) -> CostAssumptions:
    """Internal helper for `_load_assumptions`."""
...
def _business_impact(kpis: dict[str, Any], assumptions: CostAssumptions) -> dict[str, Any]:
    """Internal helper for `_business_impact`."""
...
def _overall_score(kpis: dict[str, Any]) -> dict[str, Any]:
    """Internal helper for `_overall_score`."""
    api_cov = float(kpis["api_coverage"]["coverage_pct"])
...
def _severity_from_gap(pct: float, high: float, medium: float) -> str:
    """Internal helper for `_severity_from_gap`."""
    if pct >= high:
        return "high"
    if pct >= medium:
        return "medium"
...
def _pilot_full_fixability(capability_id: str) -> dict[str, Any]:
    """Internal helper for `_pilot_full_fixability`."""
...
def _build_findings(kpis: dict[str, Any], assumptions: CostAssumptions) -> list[dict[str, Any]]:
    """Internal helper for `_build_findings`."""
...
def _findings_totals(findings: list[dict[str, Any]]) -> dict[str, Any]:
    """Internal helper for `_findings_totals`."""
    return {
        "findings_count": len(findings),
...
def _top3_gaps(reports_dir: Path) -> list[dict[str, Any]]:
    """Internal helper for `_top3_gaps`."""
...
def _capability_matrix() -> list[dict[str, Any]]:
    """Internal helper for `_capability_matrix`."""
