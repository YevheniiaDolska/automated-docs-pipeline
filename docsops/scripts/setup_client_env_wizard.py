...
MANAGED_ROOT_START = "<!-- veridoc:root-workflow:start -->"
MANAGED_ROOT_END = "<!-- veridoc:root-workflow:end -->"
...
def _parse_template(template_path: Path) -> list[tuple[str, str]]:
    """Internal helper for `_parse_template`."""
...
def _read_existing(path: Path) -> dict[str, str]:
    """Internal helper for `_read_existing`."""
    data: dict[str, str] = {}
    if not path.exists():
        return data
...
def _load_runtime(repo_root: Path) -> dict[str, Any]:
    """Internal helper for `_load_runtime`."""
    if yaml is None:
        return {}
    candidates = [
...
def _resolve_runtime_path(repo_root: Path) -> Path | None:
    """Internal helper for `_resolve_runtime_path`."""
    candidates = [
        repo_root / "docsops" / "config" / "client_runtime.yml",
        repo_root / "config" / "client_runtime.yml",
    ]
...
def _write_runtime(path: Path, runtime: dict[str, Any]) -> bool:
    """Internal helper for `_write_runtime`."""
    if yaml is None:
        return False
...
def _upsert_text_block(path: Path, block: str) -> None:
    """Internal helper for `_upsert_text_block`."""
    if path.exists():
        text = path.read_text(encoding="utf-8")
    else:
        text = ""
    if MANAGED_ROOT_START in text and MANAGED_ROOT_END in text:
        start = text.index(MANAGED_ROOT_START)
        end = text.index(MANAGED_ROOT_END) + len(MANAGED_ROOT_END)
...
def _install_root_agent_bridges(repo_root: Path, docsops_root_name: str) -> list[Path]:
    """Internal helper for `_install_root_agent_bridges`."""
    docsops_root = docsops_root_name.strip("/").strip("\\") or "docsops"
    block_lines = [
        MANAGED_ROOT_START,
        "# VeriDoc Root Routing (managed)",
        "",
        "This repository uses DocsOps-managed automation. For any docs/API request:",
        f"1. Read `{docsops_root}/AGENTS.md` (or `{docsops_root}/CLAUDE.md`) and follow it strictly.",
        "2. Do not perform ad-hoc generation.",
        f"3. Route execution through `{docsops_root}/scripts/run_autopipeline.py` and enabled entrypoints only.",
        f"4. Mandatory gates are required via `{docsops_root}/MANDATORY_RUN_GATES.md` before completion.",
        MANAGED_ROOT_END,
        "",
    ]
    block = "\n".join(block_lines)
    targets = [repo_root / "AGENTS.md", repo_root / "CLAUDE.md"]
    for target in targets:
        _upsert_text_block(target, block)
    return targets


def _is_tool_available(binary: str) -> bool:
    """Internal helper for `_is_tool_available`."""
    return bool(shutil.which(binary))


def _detect_setup_prereqs(runtime: dict[str, Any], env_values: dict[str, str]) -> dict[str, Any]:
    """Internal helper for `_detect_setup_prereqs`."""
...
def _apply_local_fallbacks(runtime: dict[str, Any], prereqs: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
    """Internal helper for `_apply_local_fallbacks`."""
    updates: list[str] = []
    api_first = runtime.get("api_first", {}) if isinstance(runtime.get("api_first"), dict) else {}
...
def _prompt_yes_no(prompt: str, default_yes: bool = True) -> bool:
    """Internal helper for `_prompt_yes_no`."""
...
def _configure_git_sync(runtime: dict[str, Any], interactive: bool = True) -> tuple[dict[str, Any], list[str]]:
    """Configure pre-weekly git fetch/pull behavior in runtime config."""
    updates: list[str] = []
    if not interactive:
        return runtime, updates

    git_sync = runtime.get("git_sync", {}) if isinstance(runtime.get("git_sync"), dict) else {}
    runtime["git_sync"] = git_sync
    current_enabled = bool(git_sync.get("enabled", False))

    print("\n[env-wizard] Git sync behavior before weekly run")
    print(
        "- If enabled, weekly flow can run `git fetch` + `git pull --rebase` before checks.\n"
        "- Useful for unattended repos.\n"
        "- Risk: can conflict with strict branch governance or local uncommitted work."
    )
    enabled = _prompt_yes_no(
        "Enable auto fetch/pull before weekly run?",
        default_yes=False,
    )
    git_sync["enabled"] = bool(enabled)
    if enabled != current_enabled:
        updates.append(f"git_sync.enabled: {str(current_enabled).lower()} -> {str(enabled).lower()}")
    return runtime, updates


def _detect_linux_asset_name() -> str:
    """Internal helper for `_detect_linux_asset_name`."""
...
def _discover_ollama_bins(repo_root: Path) -> list[str]:
    """Internal helper for `_discover_ollama_bins`."""
    candidates = [
        shutil.which("ollama"),
...
def _discover_ollama_bin(repo_root: Path) -> str | None:
    """Internal helper for `_discover_ollama_bin`."""
...
def _find_reachable_ollama_bin(repo_root: Path) -> str | None:
    """Internal helper for `_find_reachable_ollama_bin`."""
...
def _latest_linux_asset_url() -> str:
    """Internal helper for `_latest_linux_asset_url`."""
...
def _shared_ollama_cache_root() -> Path:
    """Internal helper for `_shared_ollama_cache_root`."""
...
def _copy_cached_ollama_binary(install_root: Path, cache_root: Path) -> str | None:
    """Internal helper for `_copy_cached_ollama_binary`."""
...
def _install_ollama_local_linux(repo_root: Path) -> str | None:
    """Internal helper for `_install_ollama_local_linux`."""
...
def _ensure_ollama_daemon(ollama_bin: str) -> bool:
    """Internal helper for `_ensure_ollama_daemon`."""
...
def _ollama_model_present(ollama_bin: str, model: str) -> bool:
    """Internal helper for `_ollama_model_present`."""
...
def _ollama_pull_in_progress(model: str) -> bool:
    """Internal helper for `_ollama_pull_in_progress`."""
...
def _wait_for_ollama_model(ollama_bin: str, model: str, timeout_seconds: int = 4 * 60 * 60) -> bool:
    """Internal helper for `_wait_for_ollama_model`."""
    start = time.time()
...
def _guidance_for_key(key: str) -> str:
    """Internal helper for `_guidance_for_key`."""
...
def _install_ollama_and_model(model: str, repo_root: Path | None = None) -> bool:
    """Internal helper for `_install_ollama_and_model`."""
...
def bootstrap_local_llm_runtime(repo_root: Path, runtime: dict[str, Any], interactive: bool = True) -> tuple[bool, str]:
    """Execute `bootstrap_local_llm_runtime` workflow."""
    llm_control = runtime.get("llm_control", {}) if isinstance(runtime.get("llm_control"), dict) else {}
    llm_mode = str(llm_control.get("llm_mode", "external_preferred")).strip().lower()
    strict_local = bool(llm_control.get("strict_local_first", llm_mode == "local_default"))
...
def _create_veridoc_modelfile(repo_root: Path, base_model: str, model_name: str = "veridoc-writer") -> Path:
    """Internal helper for `_create_veridoc_modelfile`."""
    docsops_root = repo_root / "docsops"
    sources = [
        docsops_root / "OLLAMA_SYSTEM_PROMPT.md",
        docsops_root / "LOCAL_MODEL.md",
        docsops_root / "AGENTS.md",
        docsops_root / "CLAUDE.md",
        docsops_root / "MANDATORY_RUN_GATES.md",
    ]
    chunks: list[str] = []
    seen: set[str] = set()
    for src in sources:
...
def _create_ollama_model(model_name: str, modelfile_path: Path) -> None:
    """Internal helper for `_create_ollama_model`."""
    repo_root = Path.cwd()
    ollama_bin = _find_reachable_ollama_bin(repo_root) or _discover_ollama_bin(repo_root)
    if not ollama_bin:
        print("[env-wizard] ollama is not available in PATH; skip custom model creation.")
        return
    if not _ensure_ollama_daemon(ollama_bin):
        print("[env-wizard] Ollama daemon is not reachable; skip custom model creation.")
        return
...
def _git_toplevel(start_dir: Path) -> Path | None:
    """Internal helper for `_git_toplevel`."""
...
def _resolve_docsops_runtime(git_root: Path, current_dir: Path) -> tuple[str, str] | None:
    """Internal helper for `_resolve_docsops_runtime`."""
    candidates = [
...
def main() -> int:
    ...
    docsops_root_name = "docsops" if (repo_root / "docsops").exists() else "."
    bridge_paths = _install_root_agent_bridges(repo_root, docsops_root_name)
    for bridge in bridge_paths:
        print(f"[env-wizard] root agent bridge updated: {bridge}")
    ...
    runtime, git_sync_updates = _configure_git_sync(runtime, interactive=True)
    runtime_updates = fallback_updates + git_sync_updates
    if runtime_path is not None and runtime_updates:
        if _write_runtime(runtime_path, runtime):
            print(f"[env-wizard] applied runtime updates: {len(runtime_updates)}")
            for line in runtime_updates:
