...
def _run_git(args: list[str]) -> str:
    """Internal helper for `_run_git`."""
    result = subprocess.run(["git", *args], capture_output=True, text=True, check=True)
    return result.stdout.strip()


def _previous_tag(current_tag: str) -> str | None:
    """Internal helper for `_previous_tag`."""
    try:
...
def _commits_since(range_spec: str) -> list[str]:
    """Internal helper for `_commits_since`."""
    try:
...
def build_release_pack(version: str | None) -> str:
    """Execute `build_release_pack` workflow."""
