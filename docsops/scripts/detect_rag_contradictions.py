#!/usr/bin/env python3
"""Detect high-risk knowledge contradictions before retrieval indexing."""

from __future__ import annotations

import argparse
import json
import math
import re
from collections import Counter, defaultdict
from datetime import datetime, timezone
from pathlib import Path
from typing import Any

import yaml

SENTENCE_SPLIT_RE = re.compile(r"[.!?]\s+|\n+")
LOCALE_RE = re.compile(r"^[a-z]{2,3}(?:-[a-z0-9]{2,8})*$", re.IGNORECASE)
WORD_RE = re.compile(r"[^\W_]+", re.UNICODE)
STOPWORDS = {
    "a",
    "an",
    "and",
    "are",
    "as",
    "at",
    "be",
    "by",
    "for",
    "from",
    "in",
    "is",
    "it",
    "of",
    "on",
    "or",
    "that",
    "the",
    "this",
    "to",
    "with",
    "you",
    "your",
    # Minimal non-English stopword coverage for cosine signal quality.
    "и",
    "в",
    "на",
    "с",
    "по",
    "для",
    "как",
    "de",
    "la",
    "el",
    "en",
    "para",
    "con",
    "y",
}

ENV_PORT_RE = re.compile(r"\b([A-Z][A-Z0-9_]*PORT)\b[^\n]{0,40}?(?:=|:)\s*([0-9]{2,5})")
PORT_RE = re.compile(
    r"\b(?:port|listen(?:ing)?\s+on(?:\s+port)?|default\s+port|порт|puerto|porta)\b[^\n]{0,40}?([0-9]{2,5})\b",
    re.IGNORECASE,
)
VERSION_RE = re.compile(
    r"\b(?:api\s+version|version|версия(?:\s+api)?|versi[oó]n(?:\s+api)?|vers[aã]o)\b[^\n]{0,24}?\bv?([0-9]+\.[0-9]+(?:\.[0-9]+)?)\b",
    re.IGNORECASE,
)
TIMEOUT_RE = re.compile(
    r"\b(?:timeout|time-out|ttl|replay\s+window|таймаут|время\s+ожидания|tiempo\s+de\s+espera|tempo\s+limite)\b[^\n]{0,40}?([0-9]+(?:\.[0-9]+)?)\s*(ms|milliseconds?|мс|s|sec|seconds?|сек|секунд(?:а|ы|у)?|m|min|minutes?|мин|минут(?:а|ы|у)?|h|hr|hours?|час(?:а|ов)?)\b",
    re.IGNORECASE,
)
RATE_LIMIT_RE = re.compile(
    r"\b([0-9]+(?:\.[0-9]+)?)\s*(?:requests?|reqs?|запрос(?:а|ов)?|solicitudes?|requisi[cç][oõ]es?)\s*(?:/|per|в)\s*(second|sec|minute|min|hour|hr|секунд(?:а|ы|у)?|минут(?:а|ы|у)?|час(?:а|ов)?)\b",
    re.IGNORECASE,
)
PAYLOAD_RE = re.compile(
    r"\b(?:max(?:imum)?\s+payload(?:\s+size)?|payload\s+size\s+limit|макс(?:имальный)?\s+размер\s+payload|tama[nñ]o\s+m[aá]ximo\s+de\s+payload|tamanho\s+m[aá]ximo\s+do\s+payload)\b[^\n]{0,40}?([0-9]+(?:\.[0-9]+)?)\s*(kb|mb|gb|bytes?|байт(?:а|ов)?|кб|мб|гб)\b",
    re.IGNORECASE,
)

CRITICAL_TYPES = {"port", "version", "timeout", "rate_limit", "payload_size"}

CLAIM_KEYWORDS: dict[str, dict[str, tuple[str, ...]]] = {
    "port": {
        "en": ("port", "listen", "default port"),
        "ru": ("порт",),
        "es": ("puerto",),
        "pt": ("porta",),
    },
    "version": {
        "en": ("version", "api version"),
        "ru": ("версия", "версия api"),
        "es": ("version", "versión", "versión api"),
        "pt": ("versão",),
    },
    "timeout": {
        "en": ("timeout", "time-out", "ttl", "replay window"),
        "ru": ("таймаут", "время ожидания", "окно повтора"),
        "es": ("tiempo de espera", "timeout"),
        "pt": ("tempo limite", "timeout"),
        "de": ("zeitüberschreitung", "timeout", "wartezeit"),
        "fr": ("délai d'attente", "timeout"),
    },
    "rate_limit": {
        "en": ("rate limit", "requests per", "req/s"),
        "ru": ("лимит запрос", "запросов в"),
        "es": ("límite de solicitudes", "solicitudes por"),
        "pt": ("limite de requisições", "requisições por"),
        "de": ("anfragelimit", "anfragen pro"),
        "fr": ("limite de requêtes", "requêtes par"),
    },
    "payload_size": {
        "en": ("payload size", "max payload"),
        "ru": ("размер payload", "максимальный размер"),
        "es": ("tamaño máximo de payload",),
        "pt": ("tamanho máximo do payload",),
        "de": ("maximale payload-größe", "payload-größe"),
        "fr": ("taille maximale de payload",),
    },
}


def _parse_iso8601(value: str) -> datetime | None:
    """Internal helper for `_parse_iso8601`."""
    raw = str(value).strip()
    if not raw:
        return None
    if raw.endswith("Z"):
        raw = raw[:-1] + "+00:00"
    try:
        parsed = datetime.fromisoformat(raw)
    except ValueError:
        return None
    if parsed.tzinfo is None:
        return parsed.replace(tzinfo=timezone.utc)
    return parsed


def _read_yaml(path: Path) -> dict[str, Any]:
    """Internal helper for `_read_yaml`."""
    payload = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
    return payload if isinstance(payload, dict) else {}


def _tokenize(text: str) -> list[str]:
    """Internal helper for `_tokenize`."""
    return [tok.lower() for tok in WORD_RE.findall(text) if tok and tok.lower() not in STOPWORDS]


def _normalize_locale(value: str | None, default: str = "en") -> str:
    """Internal helper for `_normalize_locale`."""
    raw = str(value or "").strip().lower().replace("_", "-")
    if not raw:
        return default
    primary = raw.split("-", 1)[0]
    if LOCALE_RE.match(primary):
        return primary
    return default


def _module_locale(module: dict[str, Any], default: str = "en") -> str:
    """Internal helper for `_module_locale`."""
    metadata = module.get("metadata", {}) if isinstance(module.get("metadata"), dict) else {}
    return _normalize_locale(
        str(module.get("language", "")).strip() or str(metadata.get("locale", "")).strip() or default,
        default=default,
    )


def _canonical_source_scope(module: dict[str, Any]) -> str:
    """Internal helper for `_canonical_source_scope`."""
    metadata = module.get("metadata", {}) if isinstance(module.get("metadata"), dict) else {}
    source_path = str(metadata.get("source_path", "")).strip()
    if not source_path:
        source_path = str(module.get("source_file", "")).strip()
    source_path = source_path.replace("\\", "/").strip("/")
    if not source_path:
        return ""
    parts = source_path.split("/")
    if parts and LOCALE_RE.match(parts[0]):
        parts = parts[1:]
    normalized = "/".join(parts)
    normalized = re.sub(r"\.md$", "", normalized, flags=re.IGNORECASE)
    normalized = re.sub(r"[^0-9A-Za-z\u0400-\u04FF/\-_.]+", "-", normalized).strip("-")
    return normalized.lower()


def _has_claim_keyword(line: str, claim_type: str, locale: str) -> bool:
    """Internal helper for `_has_claim_keyword`."""
    catalog = CLAIM_KEYWORDS.get(claim_type, {})
    text = line.lower()
    local = catalog.get(locale, ())
    if any(keyword in text for keyword in local):
        return True
    for key in ("en", "ru", "es", "pt"):
        if key == locale:
            continue
        if any(keyword in text for keyword in catalog.get(key, ())):
            return True
    return False


def _cosine_similarity(a: str, b: str) -> float:
    """Internal helper for `_cosine_similarity`."""
    left = Counter(_tokenize(a))
    right = Counter(_tokenize(b))
    if not left or not right:
        return 0.0
    dot = sum(float(left[key]) * float(right.get(key, 0.0)) for key in left)
    if dot <= 0.0:
        return 0.0
    left_norm = math.sqrt(sum(float(v) ** 2 for v in left.values()))
    right_norm = math.sqrt(sum(float(v) ** 2 for v in right.values()))
    if left_norm == 0.0 or right_norm == 0.0:
        return 0.0
    return dot / (left_norm * right_norm)


def _normalize_topic(module: dict[str, Any]) -> str:
    """Internal helper for `_normalize_topic`."""
    semantic = module.get("semantic", {}) if isinstance(module.get("semantic"), dict) else {}
    metadata = module.get("metadata", {}) if isinstance(module.get("metadata"), dict) else {}
    source_scope = _canonical_source_scope(module)
    topic = (
        str(semantic.get("topic", "")).strip()
        or str(metadata.get("heading", "")).strip()
        or str(module.get("title", "")).strip()
        or str(module.get("id", "")).strip()
        or source_scope
    )
    tokens = [t for t in _tokenize(topic) if t]
    return "-".join(tokens[:8]) or "general"


def _source_has_locale_prefix(module: dict[str, Any]) -> bool:
    """Internal helper for `_source_has_locale_prefix`."""
    metadata = module.get("metadata", {}) if isinstance(module.get("metadata"), dict) else {}
    source_path = str(metadata.get("source_path", "")).strip()
    if not source_path:
        source_path = str(module.get("source_file", "")).strip()
    source_path = source_path.replace("\\", "/").strip("/")
    if not source_path:
        return False
    first = source_path.split("/", 1)[0].lower().replace("_", "-")
    if not LOCALE_RE.match(first):
        return False
    module_locale = _module_locale(module, default="en")
    return first == module_locale or first.split("-", 1)[0] == module_locale.split("-", 1)[0]


def _scope_anchor(module: dict[str, Any]) -> str:
    # For localized mirrors (for example en/... and ru/...), anchor by canonical path
    # so cross-locale contradictions collapse into one group.
    """Internal helper for `_scope_anchor`."""
    source_scope = _canonical_source_scope(module)
    if source_scope and _source_has_locale_prefix(module):
        return source_scope
    # For non-locale paths, anchor by semantic topic to catch contradictions
    # across multiple docs that describe the same behavior.
    return _normalize_topic(module) or source_scope


def _normalize_seconds(value: float, unit: str) -> float:
    """Internal helper for `_normalize_seconds`."""
    key = unit.lower()
    if key in {"ms", "millisecond", "milliseconds", "мс"}:
        return value / 1000.0
    if key in {"s", "sec", "second", "seconds", "сек", "секунда", "секунды", "секунд"}:
        return value
    if key in {"m", "min", "minute", "minutes", "мин", "минута", "минуты", "минут"}:
        return value * 60.0
    if key in {"h", "hr", "hour", "hours", "час", "часа", "часов"}:
        return value * 3600.0
    return value


def _normalize_bytes(value: float, unit: str) -> float:
    """Internal helper for `_normalize_bytes`."""
    key = unit.lower()
    if key in {"kb", "кб"}:
        return value * 1024.0
    if key in {"mb", "мб"}:
        return value * 1024.0 * 1024.0
    if key in {"gb", "гб"}:
        return value * 1024.0 * 1024.0 * 1024.0
    if key in {"байт", "байта", "байтов"}:
        return value
    return value


def _normalize_requests_per_minute(value: float, unit: str) -> float:
    """Internal helper for `_normalize_requests_per_minute`."""
    key = unit.lower()
    if key in {"second", "sec", "секунда", "секунды", "секунд"}:
        return value * 60.0
    if key in {"hour", "hr", "час", "часа", "часов"}:
        return value / 60.0
    return value


def _extract_claims(module: dict[str, Any]) -> list[dict[str, Any]]:
    """Internal helper for `_extract_claims`."""
    content = module.get("content", {}) if isinstance(module.get("content"), dict) else {}
    text = str(content.get("docs_markdown", "")).strip()
    if not text:
        return []
    scope_anchor = _scope_anchor(module)
    module_id = str(module.get("id", "")).strip()
    metadata = module.get("metadata", {}) if isinstance(module.get("metadata"), dict) else {}
    source_path = str(metadata.get("source_path", "")).strip() or str(module.get("source_file", "")).strip()
    locale = _module_locale(module, default="en")
    claims: list[dict[str, Any]] = []

    for sentence in SENTENCE_SPLIT_RE.split(text):
        line = sentence.strip()
        if len(line) < 12:
            continue

        for match in ENV_PORT_RE.finditer(line):
            env_name = str(match.group(1)).strip().lower()
            port = str(int(match.group(2)))
            claims.append(
                {
                    "type": "port",
                    "scope_key": f"{scope_anchor}:port:{env_name}",
                    "value": port,
                    "value_display": port,
                    "module_id": module_id,
                    "source_path": source_path,
                    "evidence": line,
                    "locale": locale,
                }
            )
        if _has_claim_keyword(line, "port", locale):
            for match in PORT_RE.finditer(line):
                port = str(int(match.group(1)))
                claims.append(
                    {
                        "type": "port",
                        "scope_key": f"{scope_anchor}:port:default",
                        "value": port,
                        "value_display": port,
                        "module_id": module_id,
                        "source_path": source_path,
                        "evidence": line,
                        "locale": locale,
                    }
                )
        if _has_claim_keyword(line, "version", locale):
            for match in VERSION_RE.finditer(line):
                version = str(match.group(1)).strip().lower().lstrip("v")
                claims.append(
                    {
                        "type": "version",
                        "scope_key": f"{scope_anchor}:version:default",
                        "value": version,
                        "value_display": version,
                        "module_id": module_id,
                        "source_path": source_path,
                        "evidence": line,
                        "locale": locale,
                    }
                )
        if _has_claim_keyword(line, "timeout", locale):
            for match in TIMEOUT_RE.finditer(line):
                raw_value = float(match.group(1))
                unit = str(match.group(2)).strip().lower()
                seconds = _normalize_seconds(raw_value, unit)
                claims.append(
                    {
                        "type": "timeout",
                        "scope_key": f"{scope_anchor}:timeout:default",
                        "value": f"{seconds:.6f}",
                        "value_display": f"{raw_value:g} {unit}",
                        "module_id": module_id,
                        "source_path": source_path,
                        "evidence": line,
                        "locale": locale,
                    }
                )
        if _has_claim_keyword(line, "rate_limit", locale):
            for match in RATE_LIMIT_RE.finditer(line):
                raw_value = float(match.group(1))
                unit = str(match.group(2)).strip().lower()
                rpm = _normalize_requests_per_minute(raw_value, unit)
                claims.append(
                    {
                        "type": "rate_limit",
                        "scope_key": f"{scope_anchor}:rate_limit:default",
                        "value": f"{rpm:.6f}",
                        "value_display": f"{raw_value:g} req/{unit}",
                        "module_id": module_id,
                        "source_path": source_path,
                        "evidence": line,
                        "locale": locale,
                    }
                )
        if _has_claim_keyword(line, "payload_size", locale):
            for match in PAYLOAD_RE.finditer(line):
                raw_value = float(match.group(1))
                unit = str(match.group(2)).strip().lower()
                size_bytes = _normalize_bytes(raw_value, unit)
                claims.append(
                    {
                        "type": "payload_size",
                        "scope_key": f"{scope_anchor}:payload_size:default",
                        "value": f"{size_bytes:.6f}",
                        "value_display": f"{raw_value:g} {unit}",
                        "module_id": module_id,
                        "source_path": source_path,
                        "evidence": line,
                        "locale": locale,
                    }
                )
    return claims


def _build_contradiction(
    *,
    issue_id: int,
    scope_key: str,
    claim_type: str,
    claims: list[dict[str, Any]],
    min_similarity: float,
) -> dict[str, Any] | None:
    """Internal helper for `_build_contradiction`."""
    values: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for claim in claims:
        values[str(claim.get("value", ""))].append(claim)
    if len(values) <= 1:
        return None

    similarity = 0.0
    value_keys = list(values.keys())
    for idx, left_key in enumerate(value_keys):
        for right_key in value_keys[idx + 1 :]:
            for left_claim in values[left_key]:
                for right_claim in values[right_key]:
                    score = _cosine_similarity(str(left_claim.get("evidence", "")), str(right_claim.get("evidence", "")))
                    if score > similarity:
                        similarity = score

    severity = "critical" if claim_type in CRITICAL_TYPES and similarity >= min_similarity else "warning"
    module_ids = sorted({str(item.get("module_id", "")) for item in claims if str(item.get("module_id", "")).strip()})
    sources = sorted({str(item.get("source_path", "")) for item in claims if str(item.get("source_path", "")).strip()})
    locales = sorted({str(item.get("locale", "en")).strip().lower() for item in claims if str(item.get("locale", "")).strip()})
    value_items = []
    for normalized, entries in values.items():
        value_items.append(
            {
                "normalized": normalized,
                "display": sorted({str(e.get("value_display", normalized)) for e in entries})[0],
                "module_ids": sorted({str(e.get("module_id", "")) for e in entries if str(e.get("module_id", "")).strip()}),
                "sources": sorted({str(e.get("source_path", "")) for e in entries if str(e.get("source_path", "")).strip()}),
                "examples": [str(e.get("evidence", ""))[:220] for e in entries[:2]],
            }
        )
    value_items.sort(key=lambda item: item["normalized"])

    return {
        "id": f"contradiction-{issue_id}",
        "claim_type": claim_type,
        "scope_key": scope_key,
        "severity": severity,
        "similarity_max": round(similarity, 4),
        "module_ids": module_ids,
        "sources": sources,
        "locales": locales,
        "values": value_items,
        "recommendation": (
            "Resolve to a single source of truth and regenerate retrieval index."
            if severity == "critical"
            else "Review context and align docs if values refer to the same runtime behavior."
        ),
    }


def parse_args() -> argparse.Namespace:
    """Execute `parse_args` workflow."""
    parser = argparse.ArgumentParser(description="Detect contradictory claims before RAG indexing")
    parser.add_argument("--modules-dir", default="knowledge_modules")
    parser.add_argument("--report", default="reports/rag_contradictions_report.json")
    parser.add_argument("--stale-days", type=int, default=180)
    parser.add_argument("--min-similarity", type=float, default=0.35)
    parser.add_argument("--fail-on-critical", action="store_true")
    return parser.parse_args()


def main() -> int:
    """Execute `main` workflow."""
    args = parse_args()
    modules_dir = Path(args.modules_dir).resolve()
    report_path = Path(args.report).resolve()
    report_path.parent.mkdir(parents=True, exist_ok=True)

    if not modules_dir.exists():
        raise FileNotFoundError(f"Modules directory not found: {modules_dir}")

    modules: list[dict[str, Any]] = []
    for path in sorted(modules_dir.glob("*.yml")):
        payload = _read_yaml(path)
        if not payload:
            continue
        payload["source_file"] = str(path)
        modules.append(payload)

    active_modules = [m for m in modules if str(m.get("status", "active")).strip().lower() == "active"]
    claims: list[dict[str, Any]] = []
    for module in active_modules:
        claims.extend(_extract_claims(module))

    grouped: dict[tuple[str, str], list[dict[str, Any]]] = defaultdict(list)
    for claim in claims:
        key = (str(claim.get("type", "")), str(claim.get("scope_key", "")))
        grouped[key].append(claim)

    contradictions: list[dict[str, Any]] = []
    issue_id = 1
    for (claim_type, scope_key), group_claims in sorted(grouped.items(), key=lambda item: item[0]):
        issue = _build_contradiction(
            issue_id=issue_id,
            scope_key=scope_key,
            claim_type=claim_type,
            claims=group_claims,
            min_similarity=float(max(0.0, args.min_similarity)),
        )
        if issue is None:
            continue
        contradictions.append(issue)
        issue_id += 1

    now = datetime.now(timezone.utc)
    stale_candidates: list[dict[str, Any]] = []
    stale_days = max(1, int(args.stale_days))
    for module in active_modules:
        metadata = module.get("metadata", {}) if isinstance(module.get("metadata"), dict) else {}
        updated_at = _parse_iso8601(str(metadata.get("updated_at", "")).strip())
        last_verified = _parse_iso8601(str(module.get("last_verified", "")).strip())
        baseline = updated_at or last_verified
        if baseline is None:
            continue
        age_days = int((now - baseline).total_seconds() // 86400)
        if age_days > stale_days:
            stale_candidates.append(
                {
                    "module_id": str(module.get("id", "")),
                    "source_path": str(metadata.get("source_path", "")),
                    "updated_at": str(metadata.get("updated_at", "")),
                    "last_verified": str(module.get("last_verified", "")),
                    "age_days": age_days,
                }
            )

    critical_count = sum(1 for issue in contradictions if str(issue.get("severity")) == "critical")
    warning_count = sum(1 for issue in contradictions if str(issue.get("severity")) == "warning")
    critical_module_ids = sorted(
        {
            module_id
            for issue in contradictions
            if str(issue.get("severity")) == "critical"
            for module_id in issue.get("module_ids", [])
            if str(module_id).strip()
        }
    )
    status = "ok"
    if critical_count > 0:
        status = "critical"
    elif warning_count > 0 or stale_candidates:
        status = "warning"

    report = {
        "status": status,
        "generated_at": now.isoformat(),
        "modules_dir": str(modules_dir),
        "modules_scanned": len(modules),
        "active_modules_scanned": len(active_modules),
        "claims_extracted": len(claims),
        "groups_checked": len(grouped),
        "contradictions": contradictions,
        "critical_module_ids": critical_module_ids,
        "stale_candidates": sorted(stale_candidates, key=lambda item: int(item.get("age_days", 0)), reverse=True),
        "summary": {
            "critical_contradictions": critical_count,
            "warning_contradictions": warning_count,
            "stale_candidates": len(stale_candidates),
        },
    }
    report_path.write_text(json.dumps(report, ensure_ascii=True, indent=2) + "\n", encoding="utf-8")

    print(
        "[rag-contradictions] "
        f"status={status} critical={critical_count} warning={warning_count} "
        f"stale_candidates={len(stale_candidates)} report={report_path}"
    )
    if args.fail_on_critical and critical_count > 0:
        return 2
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
