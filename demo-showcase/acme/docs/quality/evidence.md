...
This page provides the complete quality evidence generated by the VeriDoc pipeline for the Acme API documentation site. It covers KPI metrics, protocol gate results, 24 automated quality checks, RAG retrieval readiness, and all pipeline artifacts.

!!! success "Powered by VeriDoc"
    This page is generated and maintained by the VeriDoc documentation pipeline.
    All metrics come from actual pipeline reports in `reports/acme-demo/`, generated on 2026-03-19.
...
| gRPC | `contracts/grpc/acme.proto` (Proto3) | **FAIL** | 0/8 | `protoc` compiler not installed |
...
!!! warning "gRPC failure"
    The gRPC protocol fails because the `protoc` compiler is not available in the build environment.
    Fix: `apt-get install -y protobuf-compiler` and re-run the pipeline.
    See [Troubleshooting](../guides/troubleshooting.md#contract-validation-fails-for-grpc) for details.
